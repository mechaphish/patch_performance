...
def compute_patch_reputation(target_cs):
    """
        Compute patch reputation for all patched binaries of given CS.
        This will update DB with results.
    :param target_cs: CS for which patch reputation needs to be computed.
    :return: None.
    """
    l.info("Trying to compute patch reputation for CS:" + str(target_cs.id))
    ...
    if len(original_cbs_perf) > 0:
        l.warning("No polls have been evaluated against original binary. "
                  "Ignoring this round of patch reputation computation.")
        return
